...
namespace Game
{
	...
	{
		// Token: 0x0600082A RID: 2090 RVA: 0x00030190 File Offset: 0x0002E390
		public PlayerScreen()
		{
			XElement node = ContentManager.Get<XElement>("Screens/PlayerScreen", null);
			...
			this.m_nameTextBox.FocusLost += delegate(TextBoxWidget <p0>)
			{
				if (this.VerifyName())
				{
		...
		// Token: 0x0600082B RID: 2091 RVA: 0x00030318 File Offset: 0x0002E518
		public override void Enter(object[] parameters)
		{
			this.m_mode = (PlayerScreen.Mode)parameters[0];
			this.m_playerData = ((this.m_mode == PlayerScreen.Mode.Edit) ? ((PlayerData)parameters[1]) : new PlayerData((Project)parameters[1]));
		...
		// Token: 0x0600082C RID: 2092 RVA: 0x0003045D File Offset: 0x0002E65D
		...
		// Token: 0x0600082D RID: 2093 RVA: 0x00030474 File Offset: 0x0002E674
		public override void Update()
		{
			...
			this.m_playerClassButton.Text = LanguageControl.Get(new string[]
			{
				base.GetType().Name,
				this.m_playerData.PlayerClass.ToString()
			});
			...
			if (this.m_characterSkinButton.IsClicked)
			{
				...
				ListSelectionDialog dialog = new ListSelectionDialog(LanguageControl.Get(base.GetType().Name, 1), items, 64f, delegate(object item)
				{
					XElement node = ContentManager.Get<XElement>("Widgets/CharacterSkinItem", null);
			...
			{
				DialogsManager.ShowDialog(null, new ListSelectionDialog(LanguageControl.Get(base.GetType().Name, 2), this.m_inputDevices, 56f, (object d) => PlayerScreen.GetDeviceDisplayName((WidgetInputDevice)d), delegate(object d)
				{
			...
			{
				DialogsManager.ShowDialog(null, new MessageDialog(LanguageControl.Warning, LanguageControl.Get(base.GetType().Name, 3), LanguageControl.Ok, LanguageControl.Cancel, delegate(MessageDialogButton b)
				{
		...
		// Token: 0x0600082E RID: 2094 RVA: 0x000308BC File Offset: 0x0002EABC
		...
		// Token: 0x0600082F RID: 2095 RVA: 0x00030A20 File Offset: 0x0002EC20
		public bool VerifyName()
		{
			...
			DialogsManager.ShowDialog(null, new MessageDialog(LanguageControl.Error, LanguageControl.Get(base.GetType().Name, 12), LanguageControl.Ok, null, null));
			return false;
		}
